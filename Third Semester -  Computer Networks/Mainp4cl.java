...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            // Trimit lungimea șirului la server
            int length2 = sir2.length() + 1;
            sendUnsignedShort(length2, socket);

            // Trimit șirul de caractere la server
            sendString(sir2 + "\0", socket);

            int length3 = sir.length() + sir2.length() + 1;
            // Primesc sirul oglindit de la server
            receiveString(socket, length3);
        } catch (IOException e) {
    ...
    private static void receiveString(Socket socket, int length) throws IOException {
        ...
        if (bytesRead > 0) {
            ...
            System.out.println("Șirul oglindit primit de la server: " + sirinterclasat);
        } else {
